...
def cypher_execution(query, parameters=None):
    ...
    # graph = Graph(NEO4J_URL, auth=(NEO4J_USER, NEO4J__PASSWORD))

    # graph = Graph("bolt://localhost:7687", auth=("neo4j", "sw2201!@#"))
    graph = Graph("bolt://neo4j:7687", auth=("neo4j", "sw2201!@#"))
...
def link_car_and_next_cars(project_name, spec_json):
    number_of_actions = len(spec_json["omsObject"]["actions"])
    for index_actions in range(number_of_actions):
        number_of_cars = len(spec_json["omsObject"]["actions"][index_actions]["cars"])    
        for index_cars in range(number_of_cars):
...
def insert_all_oms_to_gdb(oms_directory: str, user_identifier=None, project_name = None): 
    deleted = False   
    ...
    # now all cars are in, let's add next cars to gdb
    for entry in entries:
        full_path = os.path.join(oms_directory, entry)
        spec_json = seagent_file.oms_load(full_path)
        project_name = st.session_state.app_state["project_name"]
        link_car_and_next_cars(project_name, spec_json)
