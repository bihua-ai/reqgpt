...
def find_spec_file_add_action(spec_path, action_name, action_identifier):
    try:
        spec_json = seagent_file.oms_load(spec_path)
        new_action = {
...
def action_create(user_identifier, project_name, action_identifier, action_name):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_add_action(item_path, action_name, action_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def create():
    ...
    if return_element["node_type"] == "action":
        action_create(user_identifier, project_name, return_element["node_oms_identifier"], return_element["node_new_name"])
