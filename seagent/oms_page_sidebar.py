...
import oms_page_login, oms_component_navigation, oms_library_project, oms_page_project, oms_page_home, oms_page_action, oms_page_variable, oms_page_testset, oms_page_specification_generation, oms_library_specification, oms_page_upload, seagent_file
import json
...
def set_open_page():
    ...
    if return_node["node_type"] == "user":
        ...
        st.session_state.app_state["page_visible"] = "specifications page"
    ...
    #     # oms_page_specification.open()
        ...
        st.session_state.app_state["page_visible"] = "documents page"
    else:
        pass
...
def add():
    with st.sidebar:
        ...
        if st.session_state.app_state["user_identifier"] == "admin":
            return

        project_tree = []
        if "tree_data" not in st.session_state or st.session_state["tree_data"] == None:
            project_tree = oms_library_project.build_project_tree_json(st.session_state.app_state["user_identifier"])
    ...
    # };
        ...
        if return_node_raw is not None:            
            ...
            st.session_state.app_state["navtree_return_element"] = return_node # next node if accepted
            ...
            if return_node["reason"] == "created":
                ...
                current_page_node = st.session_state.app_state["current_navtree_return_element"]
                # if current_page_node["node_type"] == "action" or current_page_node["node_type"] == "variable":
                    # st.session_state.app_state["navtree_return_element"] = st.session_state.app_state["current_navtree_return_element"]
                project_tree = oms_library_project.build_project_tree_json(st.session_state.app_state["user_identifier"])
                st.session_state["tree_data"] = project_tree
                ...
                # variable_identifier, project_name = oms_library_specification.generate_variable_states()
                # oms_page_variable.open(variable_identifier, project_name)
