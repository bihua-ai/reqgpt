...
def project_create(user_identifier, project_name):
    ...
    if not os.path.exists(project_home):
        ...
        os.makedirs(os.path.join(project_home, "input_documents"))
        os.makedirs(os.path.join(project_home, "specifications"))
...
def document_delete(user_identifier, project_name, document_name):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        document_home = os.path.join(APP_DATA_HOME, user_identifier, project_name, INPUT_DOCUMENT_SUB_DIRECTORY)
        
        # Define the path to the specific document to be deleted
        document_path = os.path.join(document_home, document_name)
        
        # Check if the document home directory exists
        if os.path.exists(document_home):
...
def specification_create(user_identifier, project_name, specification_identifier, oms_object_name):
    try:
        ...
        if os.path.exists(specification_home):
            specification_file_name = f"{oms_object_name}_{specification_identifier}.json"
            path = os.getenv("SPEC_TEMPLATE_FILE_PATH")
            ...
            spec_template_json["omsObject"]["identifier"] = seagent_general.generate_uuid()
            target_path = os.path.join(specification_home, specification_file_name)
            seagent_file.oms_save(target_path, spec_template_json)
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
    except Exception as e:
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                ...
                if get_filename_from_oms_object_name(item_path, oms_object_name):
                    tbd_spec_path = item_path
                    break
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if item_path == tbd_spec_path:
                    continue
                delete_unlinked_spec(item_path, tbd_spec_path)
            os.unlink(tbd_spec_path)
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
    except Exception as e:
...
def find_spec_file_add_action(spec_path, page_name, action_name, action_identifier):
    try:
        spec_json = seagent_file.oms_load(spec_path)
        if spec_json["omsObject"]["name"] != page_name:
            return False
        new_action = {
    ...
    try:
        ...
        for i in range(number_of_actions):
            if spec_json["omsObject"]["actions"][i]["identifier"] == action_identifier:
                car_list = spec_json["omsObject"]["actions"][i]["cars"]
...
def find_spec_file_delete_variable(spec_path, variable_identifier):
    try:
        ...
        for j in range(number_of_actions):
            number_of_variables = len(spec_json["omsObject"]["actions"][j]["variables"])
            for k in range(number_of_variables):
                if (spec_json["omsObject"]["actions"][j]["variables"][k] == variable_identifier):
                    del spec_json["omsObject"]["actions"][j]["variables"][k]
                    break
        spec_json = seagent_file.oms_save(spec_path, spec_json)
        return True
    except Exception as e:
...
def find_spec_file_delete_all_variables(spec_path):
    try:
        ...
        number_of_actions = len(spec_json["omsObject"]["actions"])
        for i in range(number_of_actions):
            spec_json["omsObject"]["actions"][i]["variables"] = []
        spec_json = seagent_file.oms_save(spec_path, spec_json)
        return True
    except Exception as e:
...
def find_spec_file_add_variable(spec_path, action_identifier, variable_name, variable_identifier):
    try:
        spec_json = seagent_file.oms_load(spec_path)
        number_of_actions = len(spec_json["omsObject"]["actions"])
        found = False
        for j in range(number_of_actions):
            if action_identifier == spec_json["omsObject"]["actions"][j]["identifier"]:
                found = True
                break
        ...
        for i in range(number_of_actions):
            if spec_json["omsObject"]["actions"][i]["identifier"] == action_identifier:
                spec_json["omsObject"]["actions"][i]["variables"].append(variable_identifier)
        spec_json = seagent_file.oms_save(spec_path, spec_json)
        return True
    except Exception as e:
...
def action_create(user_identifier, project_name, page_name, action_identifier, action_name):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_add_action(item_path, page_name, action_name, action_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def delete_unlinked_next_cars_from_one_spec(spec_path, car_list):
    spec_json = seagent_file.oms_load(spec_path)
    number_of_actions = len(spec_json["omsObject"]["actions"])
    for i in range(number_of_actions):
        number_of_cars = len(spec_json["omsObject"]["actions"][i]["cars"])
        for j in range(number_of_cars):
            number_of_next_cars = len(spec_json["omsObject"]["actions"][i]["cars"][j]["nextCar"])
            for car_index in range(number_of_next_cars - 1, -1, -1):
                if spec_json["omsObject"]["actions"][i]["cars"][j]["nextCar"][car_index] in car_list:
                    del spec_json["omsObject"]["actions"][i]["cars"][j]["nextCar"][car_index]


def delete_unlinked_spec(impacted_spec_path, tbd_spec_path):
    tbd_spec_json = seagent_file.oms_load(tbd_spec_path)
    number_of_actions = len(tbd_spec_json["omsObject"]["actions"])
    car_list = []
    for i in range(number_of_actions):
        car_list.append(tbd_spec_json["omsObject"]["actions"][i]["cars"] )
    delete_unlinked_next_cars_from_one_spec(impacted_spec_path, car_list)


def action_delete(user_identifier, project_name, action_identifier):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            car_list = None
            file_with_this_car_list = None
            # Remove the contents of the document directory
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                car_list = find_spec_file_delete_action(item_path, action_identifier)
                if car_list:
                    file_with_this_car_list = item_path
                    break
            for item in os.listdir(specification_home):
                if item == file_with_this_car_list:
                    continue
                delete_unlinked_next_cars_from_one_spec(item, car_list)
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
    except Exception as e:
...
def variable_create(user_identifier, project_name, action_identifier, variable_identifier, variable_name):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_add_variable(item_path, action_identifier, variable_name, variable_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def variable_delete(user_identifier, project_name, variable_identifier):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_delete_variable(item_path, variable_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
        ...
        # Handle any exception that occurs and print the error message
        print(f"An error occurred while deleting variable: {e}")
        return False  # Return False to indicate failure due to exception
...
def is_owner_project_name_variable_identifier(variable_identifier, user_identifier, project_name):
    
    load_dotenv()
    APP_DATA_HOME = os.getenv("APP_DATA_HOME")
    SPEC_SUB_DIRECTORY = os.getenv("SPEC_SUB_DIRECTORY")
    spec_json_home = os.path.join(APP_DATA_HOME, user_identifier, project_name, SPEC_SUB_DIRECTORY)
    if not os.path.exists(spec_json_home):
        return None
...
def get_spec_file_path_from_variable_identifier(variable_identifier, user_identifier, project_name):
...
def is_owner_project_name_action_identifier(action_identifier, user_identifier, project_name):
...
def get_variable_name(spec_json, variable_identifier):
    variable_name = None
    number_variables = len(spec_json["omsObject"]["memberObjects"])
    for i in range(number_variables):
        if spec_json["omsObject"]["memberObjects"][i]["identifier"] == variable_identifier:
            variable_name = spec_json["omsObject"]["memberObjects"][i]["name"]
            break
    ...
    with os.scandir(user_root_path) as entries: # data/apps/user_id/projects/project
        for entry in entries:
            ...
            if entry.is_dir():
                ...
                project_name = projects_tree["children"][project_index]["name"]
                # print(project_name)
                ...
                for file_entry in os.scandir(specification_directory):
                    ...
                    if file_entry.is_file():
                        ...
                        for j in range(action_len):
                            ...
                            projects_tree["children"][project_index]["children"][1]["children"][page_index]["children"].append(new_action)
                            ...
                            if 'variables' in spec_json['omsObject']['actions'][j]:
                                ...
                                for k in range(variable_len):
                                    variable_identifier = spec_json["omsObject"]["actions"][j]["variables"][k]
                                    new_variable = {
                                    ...
                                    projects_tree["children"][project_index]["children"][1]["children"][page_index]["children"][j]["children"].append(new_variable)
                                    # print("place holder")
...
def create():
    ...
    if return_element["node_type"] == "project":
        project_create(user_identifier, return_element["node_oms_identifier"])
        ...
        action_create(user_identifier, project_name, return_element["node_old_name"], return_element["node_oms_identifier"], return_element["node_new_name"])
        # st.session_state.app_state["page_visible"] = "project page"
        st.session_state.app_state["page_visible"] = "information page"

    elif return_element["node_type"] == "variable":
        variable_create(user_identifier, project_name, return_element["node_oms_identifier"], return_element["node_old_name"], return_element["node_new_name"])
        # st.session_state.app_state["page_visible"] = "project page"
        st.session_state.app_state["page_visible"] = "information page"

    elif return_element["node_type"] == "specifications":
        # st.session_state.app_state["page_visible"] = "project page"
        st.session_state.app_state["page_visible"] = "information page"



def action_rename(user_identifier, project_name, action_identifier, action_name_new):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_rename_action(item_path, action_identifier, action_name_new):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def variable_rename(user_identifier, project_name, variable_identifier, action_name_new):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_rename_variable(item_path, variable_identifier, action_name_new):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def rename():
    ...
    user_identifier = st.session_state.app_state["user_identifier"]
    project_name = return_element["project_name"]
    old_name = return_element["node_old_name"]
    new_name = return_element["node_new_name"]

    if return_element["node_type"] == "project":
        project_rename(user_identifier, old_name, new_name)
    
    elif return_element["node_type"] == "page":
        specification_oms_object_rename(user_identifier, project_name, old_name, new_name)
    
    elif return_element["node_type"] == "action":
        action_rename(user_identifier, project_name, return_element["node_oms_identifier"], new_name)

    elif return_element["node_type"] == "variable":
        variable_rename(user_identifier, project_name, return_element["node_oms_identifier"], new_name)
...
def check():
    ...
    if return_element["node_type"] == "project":
        ...
        st.session_state.app_state["checked_elements"]["checked_actions"] = return_element
...
def delete():
    ...
    if return_element["node_type"] == "project":
        project_delete(user_identifier, return_element["node_oms_identifier"])
        # st.session_state.app_state["page_visible"] = "project page"
        st.session_state.app_state["page_visible"] = "information page"

    elif return_element["node_type"] == "document":
        document_delete(user_identifier, project_name, return_element["node_new_name"])
        # st.session_state.app_state["page_visible"] = "project page"
        st.session_state.app_state["page_visible"] = "information page"
