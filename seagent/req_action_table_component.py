...
def st_action_component(action_identifier, variable_list_json, spec_json, specs_json, key=None, default=None):
    """Create a new instance of "st_action_component"."""
    return _component_func(action_identifier=action_identifier, variable_list_json=variable_list_json, spec_json=spec_json, specs_json=specs_json, key=key, default=None)
...
def process_car(car_wraper, action_identifier, spec_json):
    ...
    if reason == "updated":
        for j in range(number_of_cars):
            if spec_json["omsObject"]["actions"][i]["cars"][j]["identifier"] == car_wraper["car"]["identifier"]:
                spec_json["omsObject"]["actions"][i]["cars"][j] = car_wraper["car"]
                break
        ...
        spec_json["omsObject"]["actions"][i]["cars"].append(car_wraper["car"])
