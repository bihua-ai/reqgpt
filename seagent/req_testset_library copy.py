...
# def req_gdb.cypher_execution(query, parameters=None):
...
def get_list_of_start_nodes(project_name, testcase_scope):
    ...
    """
    result = req_gdb.cypher_execution(query, {"project_name": project_name})
...
def add_common_upstream(project_name):
    ...
    """
    req_gdb.cypher_execution(query_create, {"project_name": project_name})
    ...
    """
    req_gdb.cypher_execution(query_link, {"project_name": project_name})
    ...
    DELETE r
    """
    req_gdb.cypher_execution(query_delete, {"project_name": project_name})
...
def get_all_paths_as_json(start_identifier, end_identifier, project_name):
    ...
    RETURN p
    """
    result = req_gdb.cypher_execution(query, {"start_identifier": start_identifier, "end_identifier": end_identifier, "project_name": project_name})
    print(result)
...
def convert_to_testset_json_from_gdb_output(paths_json, spec_json_home):

    # build car hashtable
    car_hashtable = {}
    variable_attribute_eqc_hasttable = {} # identifier - name or description
    state_hashtable = {}
        ...
        if file_entry.is_file():
            ...
            for i in range(len(spec_json["omsObject"]["actions"])):
                ...
                for j in range(number_of_cars):
                    ...
                    element = {}
                    element["action_name"] = spec_json["omsObject"]["actions"][i]["name"]
                    element["car"] = spec_json["omsObject"]["actions"][i]["cars"][j]
                    car_hashtable[car_identifier] = element
    ...
    for i in range(number_of_testcases):
        testcase = {}
        testcase["test_case_name"] = "将被LLM总结: 此testcase包含内容"
        testcase["test_steps"] = []

        number_of_steps = len(paths_json[i])
        for j in range(number_of_steps - 1): # skip first (common_upstream) and last (common_downstream)
            ...
            element = car_hashtable[car_identifier]
            teststep["car_name"] = element["car"]["name"]
            teststep["condition"] = element["car"]["condition"]

            teststep["action_name"] = element["action_name"]

            teststep["result"] = element["car"]["result"]
...
def generate_testset_json_from_gdb(specification_directory, project_name, testcase_scope):
    ...
    try:
        ...
        testset_json = convert_to_testset_json_from_gdb_output(paths, specification_directory)
    ...
    for index_test_cases in range(number_of_testcases):
        ...
        for index_test_steps in range(number_of_test_steps):
            ...
            for index_condition_states in range(number_of_condition_states):
                variable = testset_json["test_cases"][index_test_cases]["test_steps"][index_test_steps]["condition"]["states"][index_condition_states]
                state_name = testset_json["test_cases"][index_test_cases]["test_steps"][index_test_steps]["condition"]["states"][index_condition_states]["state_name"]
                condition_content = condition_content + f"{variable}: {state_name}\n"

                number_of_attribute_equivalence_classes = len(testset_json["test_cases"][index_test_cases]["test_steps"][index_test_steps]["condition"]["states"][index_condition_states]["attribute_equivalence_classes"])
...
def generate_testset(project_path: str, user_id: str, testcase_scope:json):
    ...
    project_name = req_gdb.insert_all_oms_to_gdb(specification_directory, user_id)
    if project_name is None:
        return None
    # generate testset from gdb
    testset_json = generate_testset_json_from_gdb(specification_directory, project_name, testcase_scope)
