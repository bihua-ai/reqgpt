...
def specification_oms_object_rename(user_identifier, project_name, oms_old_name, oms_new_name):
    try:
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if oms_object_rename(item_path, oms_object_name_old, oms_object_name_new):
                    return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
    except Exception as e:
...
def find_spec_file_delete_action(spec_path, oms_object_name, action_identifier):
    spec_json = seagent_file.oms_load(spec_path)
...
def find_spec_file_delete_variable(spec_path, oms_object_name, variable_identifier):
    spec_json = seagent_file.oms_load(spec_path)
...
def find_spec_file_rename_variable(spec_path, oms_object_name, variable_identifier, variable_name_new):
    spec_json = seagent_file.oms_load(spec_path)
...
def find_spec_file_delete_all_variables(spec_path, oms_object_name):
    spec_json = seagent_file.oms_load(spec_path)
...
def find_spec_file_add_variable(spec_path, oms_object_name, variable_name, variable_identifier):
    spec_json = seagent_file.oms_load(spec_path)
...
def action_delete(user_identifier, project_name, oms_object_name, action_name, action_identifier):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_delete_action(item_path, oms_object_name, action_name, action_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
...
def variable_delete(user_identifier, project_name, oms_object_name, variable_name, variable_identifier):
    try:
        # Load environment variables from a .env file
        load_dotenv()
        ...
        if os.path.exists(specification_home):
            ...
            for item in os.listdir(specification_home):
                item_path = os.path.join(specification_home, item)
                if find_spec_file_delete_variable(item_path, oms_object_name, variable_name, variable_identifier):
                    break
            return True  # Return True to indicate success
        else:
            return False  # Return False because the directory did not exist
                ...
                for file_entry in os.scandir(specification_directory):
                    ...
                    if file_entry.is_file():
                        ...
                        new_page = {
                            "name": page_name,
                            "id": page_id,
                            "parent_id": new_specifications["id"],
                            "node_type": "page",
                            "children": [] 
                        }
...
def rename():
    ...
    if return_element["node_type"] == "variable":
        variable_rename(user_identifier, project_name, return_element["node_oms_identifier"])
