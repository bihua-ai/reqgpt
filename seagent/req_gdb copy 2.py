...
def create_one_oms(full_path_to_spec_json, user_identifier=None):
    ...
    print(f"create_one_oms = {project_name}  {len(project_name)}")
...
def insert_all_oms(oms_directory: str, user_identifier=None): 
    deleted = False   
    spec_json = None
    ...
    for entry in entries:
        ...
        if deleted == False: 
            ...
            remove_project_from_gdb(project_name)
            deleted = True
        create_one_oms(full_path, user_identifier=user_identifier)
...
def get_start_cars():
    pass
def generate_testcase_json(project_path, user_id, testcase_scope):
    # get start cars - 
