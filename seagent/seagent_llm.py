...
def add_spec_info_till_equivalence_classes(page_spec_json):
    ...
    # add actions
    page_name_with_description = page_spec_json["omsObject"]["name"] + "(" + page_spec_json["omsObject"]["description"] + ")"
    # action_list = create_action_list(page_name)
    action_list = create_action_list(page_name_with_description)
    ...
    for i in range(len(action_list)):
        ...
        action_name_with_description = action_list[i]["name"] + "(" + action_list[i]["description"] + ")"
        variable_list = create_variable_list(page_name_with_description, action_name_with_description)

        vatiable_name_list_for_one_action = []
        for name_index in range(len(variable_list)):
            vatiable_name_list_for_one_action.append(variable_list[name_index]["identifier"])
        action_list[i]["variables"] = []
        action_list[i]["variables"] = vatiable_name_list_for_one_action
        ...
        if variable_list is None:
            ...
            page_spec_json["omsObject"]["actions"].append(action_list[i])
            continue
        ...
        for j in range(len(variable_list)):
            ...
            print(f"var name = {n0}")
            variable_name_with_description = variable_list[j]["name"] + "(" + variable_list[j]["description"] + ")"
            # attribute_list = create_attribute_list(page_name, action_list[i]["name"], variable_list[j]["name"])
            attribute_list = create_attribute_list(page_name_with_description, action_name_with_description, variable_name_with_description)
            ...
            for k in range(len(attribute_list)):
                ...
                attribute_name_with_description = attribute_list[k]["name"] + "(" + attribute_list[k]["description"] + ")"

                # equivalence_class_list = create_equivalence_class_list(page_name, action_list[i]["name"], variable_list[j]["name"], attribute_list[k]["name"])
                equivalence_class_list = create_equivalence_class_list(page_name_with_description, action_name_with_description, variable_name_with_description, attribute_name_with_description)
